...
macro_rules! fixed_node {
    ($fn_name:ident < $T:ty > ( $request:ident, $client_cmd_tx:ident $(, $app_state:ident)?) {
...

        {
            ...
            async fn $fn_name($request: ::shvrpc::rpcmessage::RpcMessage, $client_cmd_tx: $crate::ClientCommandSender<$T> $(, $app_state: Option<$T>)?) {
                ...
                if $request.shv_path().unwrap_or_default().is_empty() {
                    ...
                    async fn handler($request: ::shvrpc::rpcmessage::RpcMessage, $client_cmd_tx: $crate::ClientCommandSender<$T> $(, $app_state: $T)?)
